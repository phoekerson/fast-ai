...
export const sendWelcomeEmail = async (toEmail: string, name: string) => {
    ...
    const sender = {name: "FastAI", email: "hello@demomailtrap.com"};

    await client.send({
        from: sender,
        to: [{ email: toEmail}],
        subject: "Welcome to Fast-AI !",
        html: `
        <h1> Welcome to Fast-AI, ${name}!</h1>
        <p>We're excited to have you on board. Get started by trying generate your own content </p>`,
    });
};